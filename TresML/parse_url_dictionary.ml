...
(** [parse_url_dictionary val_of_string url_data s] Converts a url-encoded dictionary to a [StringMap].
  Adds each new binding to [s] and returns the updated [s].
  Interpret each value according to [val_of_string] : it must return a type and a value.
  Example: [parse_url_dictionary "METHOD&key1=val1&key2=val2&key3=val3"] returns a map associating [method_keyi] to [vali] *)
let parse_url_dictionary (value_of_string : string -> value * ml_type) (url_data : string) (prev_env : environment) (prev_typenv : modular_typing_environment)  : environment * modular_typing_environment = (* TODO maybe simplify with a String.split_on_char...*)
  ...
  let prev_subnamespace = match Environment.submap_opt module_name_of_method prev_env with
    | None -> Environment.empty
    | Some sub -> sub
  in
  let prev_typ_subnamespace = match Environment.submap_opt module_name_of_method prev_typenv with
    | None -> Environment.empty
    | Some subtyp -> subtyp
  in
  let (_, _, _, (final_map, final_typmap)) = String.fold_left begin fun (state, key_acc, value_acc, (map_acc, typmap_acc)) c -> match state with
      | ParsingValue -> if c = '&' then
          let key = string_of_char_list (List.rev key_acc) in
          let value, v_type = value_of_string (convert_url_data (string_of_char_list (List.rev value_acc))) in
          (ParsingKey, [], [], (Environment.add key value map_acc, Environment.add key v_type typmap_acc))
        else
          (ParsingValue, key_acc, c :: value_acc, (map_acc, typmap_acc))
      | ParsingKey -> if c = '=' then
          (ParsingValue, key_acc, value_acc, (map_acc, typmap_acc))
        else
          (ParsingKey, c :: key_acc, value_acc, (map_acc, typmap_acc))
    end (ParsingKey, [], [], (prev_subnamespace, prev_typ_subnamespace)) (data ^ "&") (* FIXME this ugly & appended at the end could be better *)
  in
  (Environment.add_sub module_name_of_method final_map prev_env, Environment.add_sub module_name_of_method final_typmap prev_typenv)